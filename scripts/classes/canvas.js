 ...
 */
export class Concentration5eCanvas {

  static init() {
    Hooks.on('dropCanvasData', Concentration5eCanvas.handleCanvasDrop);
  }
  ...
  static async handleCanvasDrop(canvas, dropData) {
    ...
    await Concentration5eCanvas.applyEffectsToTokens(canvas.scene.id, targets.map(target => target.id), [dropData.data]);
  ...
  static async applyEffectsToTokens(sceneId, tokenIds, effectDatas) {
    ...
    for (const targetToken of targetTokens) {
      for (const effectData of effectDatas) {
        Concentration5e.log('applyEffectsToTokens', targetToken, effectData);
